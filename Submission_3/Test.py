...
# class to preprocess the exoplanet dataset
...
# class to create the neural network model
...
def train_model_MLR(model, X_train, y_train, X_val, y_val, X_test, y_test, epochs=300, lr=0.001, patience=10):
    ...
    # Final evaluation on the test set - actual testing, rest of code is so that this can run
    model.eval()
    with torch.no_grad():
...
model_NN = ExoplanetNN(input_size=X_processed.shape[1])
model_MLR = ExoplanetMLR(input_size=X_processed.shape[1])
...
train_model(model_NN, X_train, y_train, X_val, y_val, X_test, y_test, epochs=200, lr=0.001)
train_model(model_MLR, X_train, y_train, X_val, y_val, X_test, y_test, epochs=200, lr=0.001)
# train_model(model_MLR, X_train, y_train, X_val, y_val, X_test, y_test, epochs=10000, lr=0.001) # early stop at 5000!, much later than the NN

torch.save(model_NN.state_dict(), 'NNmodel(MAIN_MODEL).pkl')
torch.save(model_MLR.state_dict(), 'MLRmodel(TESTING).pkl')